           ...
           {TEAM.map((member, idx) => (
             <RevealOnScroll key={idx} delay={idx * 100}>
               <div className="group relative cursor-default">
                 {/* Image Card */}
                 <div className="relative overflow-hidden rounded-[2rem] bg-dark-800 border border-white/10 aspect-[3/4] mb-6 shadow-2xl transition-all duration-500 group-hover:border-brand-accent/50 group-hover:shadow-[0_0_30px_rgba(204,255,0,0.15)] group-hover:-translate-y-2">
                    <img 
                      src={member.image} 
                      alt={member.name} 
                    ...
                    {/* Floating Socials */}
                    <div className="absolute bottom-4 right-4 flex flex-col gap-3 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-100">
                        {member.socials.linkedin && (
                          <a href={member.socials.linkedin} className="w-10 h-10 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full flex items-center justify-center hover:bg-brand-accent hover:text-black hover:border-brand-accent transition-all duration-300">
                             <Linkedin size={18} />
                          </a>
                        )}
                        {member.socials.twitter && (
                          <a href={member.socials.twitter} className="w-10 h-10 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full flex items-center justify-center hover:bg-brand-accent hover:text-black hover:border-brand-accent transition-all duration-300">
                             <Twitter size={18} />
                          </a>
                        )}
                    </div>
                 ...
                 {/* Text Info */}
                 <div className="pl-2">
                    <h3 className="text-2xl font-display font-bold text-white mb-1 group-hover:text-brand-accent transition-colors duration-300 flex items-center gap-2">
                        {member.name}
                    ...
                    <p className="text-sm font-medium text-gray-500 tracking-wider uppercase group-hover:text-gray-400 transition-colors">{member.role}</p>
                 </div>
               </div>
             </RevealOnScroll>
           ))}
