...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    >
      <div 
        className={`mx-auto w-full flex items-center justify-between transition-all duration-500 ${
            scrolled 
            ? 'max-w-5xl bg-dark-900/70 backdrop-blur-xl border border-white/10 rounded-full px-4 py-2 shadow-[0_10px_30px_rgba(0,0,0,0.5)]' 
            : 'max-w-7xl px-6 lg:px-8 bg-transparent'
        }`}
      >
        ...
        <div className="hidden md:flex items-center">
          <ul className={`flex items-center gap-1 ${scrolled ? 'bg-white/5 rounded-full px-2 py-1 border border-white/5' : ''}`}>
            {NAV_LINKS.map((link) => {
               ...
               return (
                <li key={link.label}>
                    ...
                    className={`
                        ...
                        ${isActive 
                            ? 'text-dark-900 bg-white shadow-[0_0_15px_rgba(255,255,255,0.3)]' 
